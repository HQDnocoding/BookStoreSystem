...
import flask_login
import wtforms
from flask import (flash, g, jsonify, redirect, request, send_file, session,
                   url_for)
...
class AuthenticatedView(ModelView):
    def is_accessible(self):
        ...
        return (
            current_user.is_authenticated and current_user.vai_tro_id == quan_ly_id.id
        )
...
class AuthenticatedNhanVienView(BaseView):
    def is_accessible(self):
        nhan_vien_id = VaiTro.query.filter(
            VaiTro.ten_vai_tro.__eq__("NHANVIEN")
        ).first()
        return (
            current_user.is_authenticated and current_user.vai_tro_id == nhan_vien_id.id
        )
...
class CashierView(AuthenticatedNhanVienView):
        ...
        return jsonify(
            {
                ...
                "total_amount": sum(
                    item["don_gia"] * item["so_luong"] for item in cart.values()
                ),
            }
        )
    ...
    def search_products(self):
        ...
        products = Sach.query.filter(
            or_(Sach.ten_sach.ilike(f"%{query}%"), Sach.id == query)
        ).all()
    ...
    def cashier(self):
        ...
        try:
            ...
            output_filename = os.path.join(
                output_dir, f"tch_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
            )

            utils.create_invoice_pdf(
                ten_kh, don_hang.ngay_tao_don, cart_list_dict, ten_nv, output_filename
            )
...
class Cashier2View(AuthenticatedNhanVienView):
    ...
    def get_order_details(self, don_hang_id):
        ...
        don_hang = DonHang.query.filter(
            and_(
                DonHang.id == don_hang_id, DonHang.trang_thai_id == waiting_status_id.id
            )
        ).first()
        if not don_hang:
        ...
        chi_tiet_don_hang = ChiTietDonHang.query.filter_by(
        ...
        sach_data = [
            {
                "ten_sach": (
                    Sach.query.get(chi_tiet.sach_id).ten_sach
                    if Sach.query.get(chi_tiet.sach_id)
                    else "Không xác định"
                ),
                "the_loai": (
                    TheLoai.query.get(
                        Sach.query.get(chi_tiet.sach_id).the_loai_id
                    ).ten_the_loai
                    if Sach.query.get(chi_tiet.sach_id)
                    and TheLoai.query.get(Sach.query.get(chi_tiet.sach_id).the_loai_id)
                    else "Không xác định"
                ),
                "so_luong": chi_tiet.so_luong,
                "don_gia": (
                    Sach.query.get(chi_tiet.sach_id).don_gia
                    if Sach.query.get(chi_tiet.sach_id)
                    else "Không xác định"
                ),
            }
        ...
        response = {
            ...
            "phuong_thuc_thanh_toan": (
                PhuongThucThanhToan.query.get(don_hang.phuong_thuc_id).ten_phuong_thuc
                if don_hang.phuong_thuc_id
                else None
            ),
            "trang_thai_don_hang": (
                TrangThaiDonHang.query.get(don_hang.trang_thai_id).ten_trang_thai
                if don_hang.trang_thai_id
                else None
            ),
            ...
            "thong_tin_nhan_hang": {
                "dien_thoai": (
                    thong_tin_nhan_hang.dien_thoai_nhan_hang
                    if thong_tin_nhan_hang
                    else None
                ),
                "dia_chi": (
                    thong_tin_nhan_hang.dia_chi_nhan_hang
                    if thong_tin_nhan_hang
                    else None
                ),
            },
    ...
    def create_invoice(self, don_hang_id):
        try:
            ...
            os.makedirs(output_dir, exist_ok=True)
            output_filename = os.path.join(
                output_dir, f"dt_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
            )

            app.logger.error(f"date {infor['don_hang_id']}")

            utils.create_invoice_pdf(
                ten_kh, infor["ngay_thanh_toan"], sach_data, ho_ten_nv, output_filename
            )
...
class RevenueStatsView(AuthenticatedStatsView):
    ...
    def export(self):
        ...
        try:
            ...
            os.makedirs(output_dir, exist_ok=True)
            output_filename = os.path.join(
                output_dir, f"tkdt_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
            )
...
class FrequencyStatsView(AuthenticatedStatsView):
    ...
    def export(self):
        try:
            ...
            os.makedirs(output_dir, exist_ok=True)
            output_filename = os.path.join(
                output_dir, f"tkts_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
            )
...
class SachForm(Form):
    ...
    bia_sach = FileUploadField(
        "Bìa sách",
        base_path="upload/bia_sach",
        validators=[
            DataRequired(),
            FileAllowed(
                ["jpg", "jpeg", "png", "gif"], "Chỉ được phép upload file hình ảnh!"
            ),
        ],
    )
...
class SachView(AuthenticatedView):
    ...
    def edit_form(self, obj):
        ...
        form.tac_gia_id.choices = [
            (a.id, a.ten_tac_gia) for a in TacGia.query.all()
    ...
    column_formatters = {
        "tac_gia_id": lambda v, c, m, p: (
            db.session.query(TacGia.ten_tac_gia)
            .filter(TacGia.id == m.tac_gia_id)
            .first()[0]
            if m.tac_gia_id
            else "Chưa có tác giả"
        ),
        "the_loai_id": lambda v, c, m, p: (
            db.session.query(TheLoai.ten_the_loai)
            .filter(TheLoai.id == m.the_loai_id)
            .first()[0]
            if m.the_loai_id
            else "Chưa có thể loại"
        ),
    }
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if form.new_the_loai.data:
            the_loai = TheLoai.query.filter_by(
                ten_the_loai=form.new_the_loai.data
            ).first()
            if not the_loai:
                # Nếu không có tác giả, tạo tác giả mới
        ...
        if (
            file_data
            and hasattr(file_data, "content_type")
            and hasattr(file_data, "filename")
        ):  # Kiểm tra nếu là file hợp lệ
            # Kiểm tra loại file
            ...
            try:
                upload_result = cloudinary.uploader.upload(
                    os.path.join("upload/bia_sach", file_data.filename),
                    folder="upload/bia_sach",
                )
...
class TacGiaView(AuthenticatedView):
    ...
    column_labels = {"id": "Mã tác giả", "ten_tac_gia": "Tên tác giả"}
...
class TheLoaiView(AuthenticatedView):
    ...
    column_labels = {"id": "Mã thể loại", "ten_the_loai": "Tên thể loại"}
...
class UserForm(FlaskForm):
    ...
    ngay_tao = ngay_tao = DateTimeField(
        "Ngày tạo", format="%Y-%m-%d %H:%M:%S", default=datetime.now
    )  # Gán ngày giờ mặc định
    avatar = FileField(
        "Avatar",
        validators=[
            FileAllowed(
    ...
    vai_tro_id = QuerySelectField(
        "Vai trò",
        query_factory=lambda: VaiTro.query.all(),
        get_label="ten_vai_tro",
        allow_blank=False,
    )
...
class UserView(AuthenticatedView):
    ...
    def on_model_change(self, form, model, is_created):
        ...
        if file_data:
            ...
            upload_result = cloudinary.uploader.upload(
                file_data, folder="upload/avatar"
            )
...
class NhapPhieuView(AuthenticatedQuanLyKhoViewBV):
    ...
    def add_to_cart(self):
        ...
        if sach.so_luong > sl_min_to_nhap:
            ...
            return jsonify(
                {
                    "error": f" Số lượng sách {data['ten_sach']} đã lớn hơn {sl_min_to_nhap} !!!"
                }
            )
    ...
    def create_invoice(self):
        ...
        for c in cart.values():
            if c["so_luong"] < sl_nhap_min:
                flash(
                    f"sách {c['ten_sach']} có số lượng nhỏ hơn {sl_nhap_min}", "error"
                )
                return redirect("/admin/nhapphieuview")
        ...
        for c in cart.values():
            ...
            if sach:  # Kiểm tra xem sach có tồn tại không
                ...
                data.append(
                    [
                        sach.ten_sach,
                        sach.the_loai.ten_the_loai,
                        sach.tac_gia.ten_tac_gia,
                        c["so_luong"],
                    ]
                )  # Chuyển Decimal thành float nếu cần
        ...
        os.makedirs(output_dir, exist_ok=True)
        output_filename = os.path.join(
            output_dir, f"Phieu_{datetime.now().strftime('%Y%m%d%H%M%S')}.pdf"
        )
...
class PhuongThucThanhToanView(AuthenticatedView):
    ...
    form_choices = {"ten_phuong_thuc": [("ONLINE", "ONLINE"), ("TIỀN MẶT", "TIỀN MẶT")]}
...
admin.add_view(
    TheLoaiView(TheLoai, db.session, name="Thể loại", category="Quản lý sách")
)
...
admin.add_view(
    TrangThaiDonHangView(
        TrangThaiDonHang, db.session, name="Trạng thái đơn hàng", category="Khác"
    )
)

admin.add_view(
    PhuongThucThanhToanView(
        PhuongThucThanhToan, db.session, name="Phương thức thanh toán", category="Khác"
    )
)
...
admin.add_view(
    XemPhieuNhapSach(
        PhieuNhapSach, db.session, name="Xem Phiếu Nhập sách", category="XEM PHIẾU NHẬP"
    )
)
admin.add_view(
    XemPhieuNhapSach(
        ...
        db.session,
        name="Xem Chi Tiết Phiếu",
        category="XEM PHIẾU NHẬP",
    )
)

admin.add_view(RevenueStatsView(name="Thống kê doanh thu", category="Thống kê báo cáo"))
admin.add_view(
    FrequencyStatsView(name="Thống kê tần suất", category="Thống kê báo cáo")
)
